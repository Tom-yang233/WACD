...
def bmp_file_in(file_path):
    ...
    color_palette = img.getpalette()  # 获取颜色表
    img_data = np.array(img)  # 获取图像数据
...
def tx_pic(file_path, RATE=48000, CHANNELS=1, _CHUNK=20000, FORMAT=pyaudio.paInt16, AMPLITUDE=4096):
    ...
    trigger_sig = sin_signal(t, freq=500, AMPLITUDE=AMPLITUDE)
